...
SS="$(oc get subs -o name -n sealed-secrets | grep sealed-secrets-operator)"
oc -n sealed-secrets wait --timeout=120s --for=condition=CatalogSourcesUnhealthy=False ${SS}
